...
function buildMatchGrid() {
    ...
    const list = MATCHES
        ...
        .filter(match => testMode ? true : now < match._end)
        .sort((a, b) => {
            const aLive = (testMode || now >= a._start) ? 0 : 1;
            const bLive = (testMode || now >= b._start) ? 0 : 1;
    ...
    grid.innerHTML = list.map(match => {
        const live = testMode || now >= match._start;
        ...
        <div class="match-card ${live ? 'is-live' : 'is-upcoming'}" onclick="${live ? `selectMatch('${match.id}')` : 'void(0)'}">
            <div class="match-card-left">
                ...
                ${!live ? `<div class="match-countdown" id="cd_${match.id}">Loading...</div>` : `<div class="match-countdown live-now">${testMode ? 'Testing unlocked' : 'Betting unlocked'}</div>`}
            </div>
            <span class="match-badge ${live ? 'badge-live' : 'badge-upcoming'}">
                ${live ? (testMode ? 'TEST LIVE' : 'LIVE') : 'Soon'}
            </span>
        </div>`;
    }).join('');

    list.filter(match => !testMode && now < match._start).forEach(match => {
    ...
    const firstLive = list.find(match => testMode || now >= match._start);
...
function selectMatch(id) {
    ...
    if (!CONFIG.TEST_MODE && new Date() < new Date(match.startTime)) {
...
async function onPlaceBet() {
    ...
    S.qrRevealTimer = setTimeout(() => {
        showQRInModal(typeof STATIC_PAYMENT_QR !== 'undefined' ? STATIC_PAYMENT_QR : 'payment-qr.jpeg');
